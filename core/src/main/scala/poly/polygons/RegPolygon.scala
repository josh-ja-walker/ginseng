...
case class RegPolygon[N <: Int](verts: Pos*)(using ValueOf[N], N >= 3)
    extends Polygon[N] {}
...
object RegPolygon {
    ...
    def apply[N <: Int](a: Pos, length: Length)(using ValueOf[N], N >= 3): RegPolygon[N] = {
        val n = valueOf[N]
    ...
    def unital[N <: Int](using ValueOf[N], N >= 3): RegPolygon[N] = RegPolygon.size(0.5d.u)
    def size[N <: Int](size: Length)(using ValueOf[N], N >= 3): RegPolygon[N] = RegPolygon(Pos.origin, size)

    def centered[N <: Int](center: Pos, size: Length)(using ValueOf[N], N >= 3): RegPolygon[N] = 
